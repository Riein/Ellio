...
import com.sasha.game.main.Resources;
import com.sasha.game.model.*;
...
public class PlayState extends State{
	...
	public void update(float delta) {
		// Check if Player is dead and set state to game over with player score
		if (!player.isAlive()) {
			setCurrentState(new GameOverState(playerScore / 100));
		}
		// Increment player score each frame, and increase block speed every 500 points
		playerScore += 1;
	...
	private void renderScore(Graphics g) {
		...
		g.drawString("" + playerScore / 100, 20, 20);
	}
	...
	private void renderBlocks(Graphics g) {
		for (Block b : blocks) {
			if (b.isVisible()) {
				g.drawImage(Resources.block,(int) b.getX(),(int) b.getY(), BLOCK_WIDTH,
						BLOCK_HEIGHT, null);
			}
		}
	}
